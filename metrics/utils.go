...
func avgRuntime(jobs []types.Job) float64 {
	if len(jobs) == 0 {
		return 0
	}
	sumRuntime := types.Time(0.)
	for _, job := range jobs {
		//JobName := job.JobName()
		for _, ranges := range job.ExecutionDetail().ExecutionRanges() {
			//fmt.Printf("  %s:\n", gpu)
			//JobGpu := gpu
			for _, r := range ranges {
				...
				sumRuntime += types.Time(Jobruntime)
			}
		}
	}
	return float64(sumRuntime / types.Time(len(jobs)))
}
