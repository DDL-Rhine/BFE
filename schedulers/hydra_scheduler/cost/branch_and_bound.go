...
func (m *BranchAndBoundTemplate) MinCost(params *MinCostParams) (float64, []types.Job) {
	...
	params1 := &MinCostParams{
		...
		GPU:         params.GPU,
		Jobs:        copied1,
		WaitingJobs: params.WaitingJobs,
	...
	for minHeap.Len() > 0 {
		...
		for _, job := range params.Jobs {
			...
			for _, newJobs := range newJobsSequences {
				newJobs := newJobs
				costResp := costSolver.Cost(gpu, newJobs, params.WaitingJobs, params.CurrTime)
...
func (m *BranchAndBoundAllPermutation) predict(
	...
	predictCostResp := params.CostSolver.Cost(params.GPU, predictJobList, params.WaitingJobs, params.CurrTime)
...
func (b *BranchAndBoundDDLInsertion) initNodes(params *BranchAndBoundMinCostParams) []*BranchAndBoundNode {
	...
	costResp := params.CostSolver.Cost(params.GPU, noDDLJobs, params.WaitingJobs, params.CurrTime)
...
func (b *BranchAndBoundDDLInsertion) predict(
	...
	predictJobList := func() []types.Job {
		...
		for i := 0; i < len(newJobs); i++ {
			...
			resp := params.CostSolver.Cost(params.GPU, resultList, params.WaitingJobs, params.CurrTime)
			if bestList == nil {
				bestList = resultList
				bestCostResp = resp
				continue
	...
	cHat = func() float64 {
		...
		resp := params.CostSolver.Cost(params.GPU, newJobs, params.WaitingJobs, params.CurrTime)
		return resp.Cost
	}()
